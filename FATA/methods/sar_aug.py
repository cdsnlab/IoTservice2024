...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema):
    ...
    if not len(x[filter_ids_1])==0:
        ...
        loss.backward()
        wandb.log({'loss/ent_filtd': loss}, commit=False)
        ...
        loss_second = entropys2[filter_ids_2].mean(0) + loss_aug
        wandb.log({'loss/ent_final': loss_second, 'loss/aug': loss_aug}, commit=False)
